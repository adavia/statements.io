...
class StatementEdit extends PureComponent {
  render() {
    const { onUpdateStatement, statement, categories, statementCategories } = this.props;
    return (
      <Formik
        initialValues={{ 
          title: statement.title, 
          description: statement.description,
          categories: statementCategories.map(c => ({ value: c._id, label: c.name }))
        }}
        validationSchema={
          Yup.object().shape({
            title: Yup.string()
        ...
        onSubmit={ async (values, actions) => {
          ...
          await onUpdateStatement({ ...values, categories });
